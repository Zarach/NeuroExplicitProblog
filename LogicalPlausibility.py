...
def numerous_grounding(grounding_rule, grounding_facts, ground_members):
...
def grounding(grounding_rule, grounding_facts):
    print(f"Grounding -> {grounding_rule}")
    number_of_variables = grounding_rule.split(":-")[0].count(",")
    label = grounding_rule.split("(")[0]
    query = f"query({label}(_{', _'*number_of_variables}))"
...
def save_facts(facts):
    script_dir = os.path.dirname(__file__)  # <-- absolute dir the script is in
    rel_path = "DataBases/Facts/facts_grounded.json"
    abs_file_path = os.path.join(script_dir, rel_path)
...
def save_facts_for_probfoil(facts, filename):
...
def save_apl_on_phases(input_filepath: str, output_filepath: str, resampling: int = 1):
    """
...
rules_list = []
rules = load_rules("rules_manual.json")
facts_string, fact_list = load_facts('facts_from_ml_sensors_finetuned_VI.json', "2023-03-13 00:00:00", "2023-03-26 23:59:59")
facts_string += grounding(grounding_rule='act(A) :- kettle(A).', grounding_facts=facts_string) # ; coffee(A); washing_machine(A); microwave(A); toaster(A); television(A).
facts_string += grounding(grounding_rule='duration(A, B) :- start(A,C), get_stamp(C, D), end(A,E), get_stamp(E, F), B is F-D.', grounding_facts=facts_string)
facts_string += grounding(grounding_rule='weekday(A, B) :- start(A,C), get_weekday(C, B).', grounding_facts=facts_string)
facts_string += grounding(grounding_rule='start_hour(A, B) :- start(A,C), get_hour(C, B).', grounding_facts=facts_string)
facts_string += numerous_grounding(grounding_rule='before_hour(A, B) :- start(A,C), get_hour(C, D), [members], D < B.', grounding_facts=facts_string, ground_members=list(range(0, 24, 1))) #list(range(0, 5000, 5)
facts_string += numerous_grounding(grounding_rule='duration_gt(A, B) :- duration(A, C), [members], C > B.', grounding_facts=facts_string, ground_members=list(range(0, 5000, 10)))


fact_string_list = [e+'.' for e in facts_string.split('. ') if e]

save_facts(fact_string_list)

if test:
    save_facts_for_probfoil(facts_string, 'test_facts.pl')
    df, fact_list_evaluated = evaluate(rules, 'test_facts.pl')


    df_active_phase = Utils.load_csv_from_folder("DataBases/Barthi/active_phases", "timestamp")[['kettle']]
    df_active_phase = df_active_phase.loc["2023-03-13 00:00:00":"2023-03-26 23:59:59"]
    pd.to_datetime(df_active_phase.index)

    df_eval = pd.concat([df, df_active_phase], axis=1).fillna(0)

    script_dir = os.path.dirname(__file__)  # <-- absolute dir the script is in
    rel_path = "Results/evaluation_plausibility_manual_VII.csv"
    abs_file_path = os.path.join(script_dir, rel_path)
    ...
    save_facts_for_probfoil(facts_string, 'learning_facts.pl')
    learned_rules = learn('learning_facts.pl')
