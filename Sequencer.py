...
class Sequencer:
    ...
    def _state_ranges(self, Series, first_val_is_target=False):
        ...
        change_ind = list(change_ind)

        #Delete last second because it is not part of the pattern
        for ind in range(0, len(change_ind), 2):
            change_ind[ind + 1] = change_ind[ind + 1] + datetime.timedelta(0,-1)
    ...
    def _check_and_cut_sequences(self, df, sequence_windows):
        ...
        for window in sequence_windows:
            sequence = df.loc[window[0]:window[-1], 'kettle']
    ...
    def sequence_input(self, dataframe, plot: bool = False):
        ...
        df['rolling_left'] = df['kettle'].rolling(f'{self.patience}s').max() > self.power_threshold
        df['rolling_right'] = df['kettle'][-1::-1].rolling(f'{self.patience}s').max()[-1::-1] > self.power_threshold
        ...
        df_active_phase = df['toBeSequenced'].astype(int)
        df_active_phase.to_csv('hund.csv')
        ...
        for series in self.pc_series_list:
            ...
            recording_time_end = series.index[-1].strftime('%H:%M:%S')
    ...
    def save_pc_patterns(self):
        ...
        if self.pc_series_list:
            with open(f'{script_dir}\\{self.output_path}{self.appliance_name}_pt{self.power_threshold}_pa{self.patience}'
            ...
            # print(f'Path: {script_dir}\\{self.output_path}{self.appliance_name}_pt{self.power_threshold}_pa{self.patience}'
            #           f'_pt{self.minimal_power_threshold}_{self.maximum_power_threshold}'
            #           f'_pl{self.minimal_pattern_lenght}_{self.maximum_pattern_lenght}'
            #           f'_tpc{self.total_power_consumption_threshold}')
        else:
            print('Error: No sequenced power consumption patterns available to be saved.')


if __name__ == '__main__':
    time_series_file = 'C:/Users/Benjamin-admin/PycharmProjects/ProbFOIL/DataBases/Barthi/power_consumption/2022-12-05.csv'
    script_dir = os.path.dirname(__file__)
    ...
    df = Utils.load_csv_from_folder(f'Barthi/power_consumption', index='timestamp')
    #plt.plot(df['kettle'])
    #plt.show()
    ...
    min_pattern_lenght = 1
    max_pattern_lenght = 43200
    total_power_consumption_threshold = 1000000  # in kwh umrechnen
    appliance_name = 'kettle'
    output_path = ''

    sequencer = Sequencer(appliance_name, time_series_file, output_path, power_threshold, patience, min_power_threshold,
    ...
    script_dir = os.path.dirname(__file__)
    with open(f'{script_dir}\\{output_path}{appliance_name}_pt{power_threshold}_pa{patience}_pt{min_power_threshold}_{max_power_threshold}'
